#ifndef UNLIT_INPUT_INCULDED
#define UNLIT_INPUT_INCLUDED
...
float4 GetBase(InputConfig c)
{
    ...
    if (c.nearFade)
    {
        float nearAttenuation =
            (c.fragment.depth - INPUT_PROP(_NearFadeDistance)) / INPUT_PROP(_NearFadeRange);
    ...
    if (c.softParticles)
    {
        ...
        float nearAttenuation =
            (depthDelta - INPUT_PROP(_SoftParticlesDistance)) / INPUT_PROP(_SoftParticlesRange);
        baseMap.a *= saturate(nearAttenuation);
    }
...
float2 GetDistortion(InputConfig c)
{
    ...
    if (c.flipbookBlending)
    {
        rawMap = lerp(rawMap, SAMPLE_TEXTURE2D(_DistortionMap, sampler_DistortionMap, c.flipbookUVB.xy),
            c.flipbookUVB.z);
    }

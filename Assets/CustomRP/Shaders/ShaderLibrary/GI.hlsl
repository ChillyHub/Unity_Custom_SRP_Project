...
// When needs to render lightmap object
...
float3 SampleLightProbe(Surface surface)
{
    ...
    return 0.0;
#else
    // Whether or not to use LPPV
    if (unity_ProbeVolumeParams.x)
    {
        return SampleProbeVolumeSH4(
            TEXTURE3D_ARGS(unity_ProbeVolumeSH, samplerunity_ProbeVolumeSH),
            surface.position, surface.normal, unity_ProbeVolumeWorldToObject,
            unity_ProbeVolumeParams.y, unity_ProbeVolumeParams.z, unity_ProbeVolumeMin.xyz,
...
// Sample shadowMask and get baked shadow data
float4 SampleBakedShadows(float2 lightMapUV, Surface surface)
{
    ...
    if (unity_ProbeVolumeParams.x)
    {
        // Sample LPPV occlusion data
        return SampleProbeOcclusion(
            TEXTURE3D_ARGS(unity_ProbeVolumeSH, samplerunity_ProbeVolumeSH),
            surface.position, unity_ProbeVolumeWorldToObject, unity_ProbeVolumeParams.y,
            unity_ProbeVolumeParams.z, unity_ProbeVolumeParams.xyz,
            unity_ProbeVolumeSizeInv.xyz);
    }
    else
    {

...
{
    float horizontal =
        abs(luma.n + luma.s - 2.0 * luma.n) * 2.0 +
        abs(luma.ne + luma.se - 2.0 * luma.e) +
        abs(luma.nw + luma.sw - 2.0 * luma.w);
    float vertical =
        abs(luma.e + luma.w - 2.0 * luma.m) * 2.0 +
...
float4 FXAAPassFragment(Varyings input) : SV_Target
{
    ...
    float blendFactor =
        max(GetSubpixelBlendFactor(luma), GetEdgeBlendFactor(luma, edge, input.screenUV));
